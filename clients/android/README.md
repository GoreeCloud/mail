...
- JVM tests for capability/governance truthfulness, bounded V1.6 presentation/context policy and exact non-secret session metadata matching, with separate fail-closed native, Glaze and session-source guards.
- SOURCE_READY Mail session-binding metadata checks referencing current GoreeCloud Identity Draft PR #9's contract-only Development source; Identity runtime and an accepted native Mail application registration remain absent, so `runtimeReady=false` even for structurally valid metadata.
...
This foundation adds no mailbox credentials, GoreeCloud Identity runtime registration, OAuth/OIDC client, provider account, network transport, local authoritative mailbox, background synchronization, push token, attachment transfer, secure message store, or production signing authority. SOURCE_READY local session metadata matching is a fail-closed prerequisite only; it is neither authenticated session evidence nor permission to access Mail-owned data.
